...
@section('title', 'รายละเอียดกลุ่มวิชาเอก: ' . $placementType->name)
    ...
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">รายละเอียด: {{ $placementType->name }}</li>
            </ol>
        </div>
    </div>
@stop
            ...
            <div class="col-md-8 offset-md-2"> {{-- จัดกึ่งกลาง --}}
                <div class="card card-info"> {{-- สี card เป็น info สำหรับหน้าแสดงรายละเอียด --}}
                    <div class="card-header">
                        <h3 class="card-title">ข้อมูลกลุ่มวิชาเอก: {{ $placementType->name }}</h3>
                        <div class="card-tools">
                            <a href="{{ route('admin.subject-groups.edit', $placementType->id) }}"
                    ...
                    <div class="card-body">
                        <dl class="row">
                            ...
                            <dd class="col-sm-8">{{ $placementType->id }}</dd>

                            <dt class="col-sm-4">ชื่อกลุ่มวิชาเอก:</dt>
                            <dd class="col-sm-8">{{ $placementType->name }}</dd>

                            <dt class="col-sm-4">รหัสกลุ่มวิชา:</dt>
                            <dd class="col-sm-8">{{ $placementType->code ?? '-' }}</dd>

                            <dt class="col-sm-4">วันที่สร้าง:</dt>
                            <dd class="col-sm-8">
                                {{ $placementType->created_at ? $placementType->created_at->format('d/m/Y H:i:s') : '-' }}
                                ({{ $placementType->created_at ? $placementType->created_at->diffForHumans() : '' }})</dd>

                            <dt class="col-sm-4">วันที่แก้ไขล่าสุด:</dt>
                            <dd class="col-sm-8">
                                {{ $placementType->updated_at ? $placementType->updated_at->format('d/m/Y H:i:s') : '-' }}
                                ({{ $placementType->updated_at ? $placementType->updated_at->diffForHumans() : '' }})</dd>
                        </dl>

                        <hr>

                        <h5 class="mt-4 mb-2">รายการบรรจุที่เกี่ยวข้องกับกลุ่มวิชาเอกนี้
                            ({{ $placementType->placementRecords()->count() }} รายการ)</h5>
                        @php
                            ...
                            // $placementRecords = $placementType->placementRecords()->with('educationalArea')->latest('announcement_date')->paginate(5);
                            $placementRecords = $placementType
                                ->placementRecords()
                        ...
                        @if ($placementRecords->count() > 0)
                            ...
                            @if ($placementType->placementRecords()->count() > $placementRecords->count())
                                <div class="text-center mt-2">
                                    <small class="text-muted">และอีก
                                        {{ $placementType->placementRecords()->count() - $placementRecords->count() }}
                                        รายการ...</small>
